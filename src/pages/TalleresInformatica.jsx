...
export default function CursosInformatica(){
  ...
  const calcularPromedioDuracion = () => {
    const duraciones = informatica.map(c => {
      // Buscar duración en diferentes campos
      const duracion = c.duracion || c.duration || c.meses || c.months
      if (duracion) {
        // Extraer números del texto
        const numeros = duracion.toString().match(/\d+/g)
        if (numeros && numeros.length > 0) {
          return parseFloat(numeros[0])
        }
      }
      // Si no hay duración específica, asumir 3 meses por defecto para informática
      return 3
    }).filter(d => d > 0)
    
    if (duraciones.length === 0) return 3
    const promedio = duraciones.reduce((sum, d) => sum + d, 0) / duraciones.length
    return Math.round(promedio * 10) / 10 // Redondear a 1 decimal
  }

  const promedioDuracion = calcularPromedioDuracion()
      ...
      <div className="container">
        {loading ? (
          ...
          <div className="text-center py-5">
            ...
            <h4 style={{ color: '#1565c0', fontWeight: '600' }}>No hay cursos disponibles</h4>
          </div>
        ) : (
          <>
            {/* Header */}
            <div className="text-center mb-5">
              ...
              <p style={{
                ...
                Domina las herramientas digitales y programas esenciales para el mundo laboral
              </p>
              <div style={{
                display: 'inline-flex',
                alignItems: 'center',
                ...
                <div className="text-center">
                  ...
                  <div style={{ fontSize: '0.9rem', color: '#5e92f3' }}>Cursos</div>
                </div>
